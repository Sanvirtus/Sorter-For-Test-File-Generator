...
public class FileSorterByChunks(long maxChunkSizeInBytes, long processorCount, bool notOnlyProgress)
{
        ...
        string inputFilePath, string outputFilePath, CancellationToken token = default)
    {
        ...
        {
            Display.PrintLineBlueText("\n[Fast Path] File is small enough. Sorting in memory");
        ...
        try
        {
            Display.PrintLineBlueText("\n[Stage 1/2] Splitting and sorting chunks");
            var tempFiles = await Stage1SplitAndSortChunksParallelAsync(inputFilePath, tempDirectory, token);

            Display.PrintLineBlueText("\nCreated temp files:");
            Display.PrintTempFiles(tempFiles);

            Display.PrintLineBlueText($"\n[Stage 2/2] Merging {tempFiles.Count} files");
        ...
        {
            Display.PrintLineRedText(e.ToString());
        }
        finally
        {
        ...
        string inputFilePath, string tempDirectory, CancellationToken token)
    {
        ...
        using (var reader = new StreamReader(inputFilePath))
        {
            while (!reader.EndOfStream)
            {
                ...
                Display.PrintProgressBar(reader.BaseStream.Position, totalLength, notOnlyProgress);
        ...
        CancellationToken token)
    {
        ...
        {
            Display.PrintLineRedText("Fast Path Error: " + e.Message);
        }
    }
}